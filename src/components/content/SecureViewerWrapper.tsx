...
export const SecureViewerWrapper = ({ children, watermarkText }: SecureViewerWrapperProps) => {
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      if (e.key === 'PrintScreen' || e.keyCode === 44) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard
            .writeText(`⚠️ COPYRIGHT VIOLATION LOGGED: User ID [${watermarkText}] ⚠️`)
      ...
      mutations.forEach((mutation) => {
        if (mutation.type === 'childList') {
          ...
          const isWatermarkRemoved = removedNodes.some(
            (node) =>
              node === watermarkRef.current || (node as HTMLElement).id === 'secure-watermark',
          );
  ...
  return (
    ...
    >
      ...
      {isBlurred && (
        <div className="absolute inset-0 z-[200] bg-white/95 backdrop-blur-2xl flex items-center justify-center">
          <div className="text-center p-6">
            ...
            <p className="text-slate-600">Content is hidden when the window loses focus.</p>
          </div>
        </div>
      )}
        ...
        {Array.from({ length: 50 }).map((_, i) => (
          ...
          >
            <div className="text-center">
              <p className="text-xl font-black text-slate-500/50 whitespace-nowrap">
                {watermarkText}
              </p>
              <p className="text-sm font-bold text-slate-500/30 whitespace-nowrap">
      ...
      {/* Content Container */}
      <div
        className={`w-full h-full transition-opacity duration-100 ${isBlurred ? 'opacity-0' : 'opacity-100'}`}
      >
        {children}
      </div>
    </div>
