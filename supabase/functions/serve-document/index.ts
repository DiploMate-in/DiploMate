...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[Serve-Document] Init. URL: ${!!supabaseUrl}, AnonKey: ${!!anonKey}, SRKey: ${!!serviceRoleKey}, AuthHeader: ${!!authHeader}`);
    ...
    if (!purchase) {
       console.error(`[Serve-Document] Purchase not found for user ${user.id} and content ${content_id}`);
       return new Response(
        JSON.stringify({ 
    ...
    if (!contentItem?.file_url) {
      return new Response(
        JSON.stringify({ error: 'Content not found or missing file URL' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    // Check for Google Drive
    if (contentItem.file_url.includes('drive.google.com') || contentItem.file_url.includes('docs.google.com')) {
        return new Response(
            JSON.stringify({ error: 'External content cannot be viewed securely', isExternal: true, url: contentItem.file_url }),
